...
def multi_stop():
    ...
    prompt = f"""Return ONLY valid JSON. No other text. User wants: "{query}"
    Departure city: {departure}
    Currency: {currency}
    
    Create a multi-stop flight itinerary that breaks long flights into shorter legs with stopovers.
    Use REAL airline codes (AC, UA, DL, AA, BA, LH, AF, KL, EK, QR, SQ, TG, CX, NH, JL, KE, OZ, BR, CI).
    ...
    Generate a realistic route based on geography. Use major hub airports."""
    ...
    # No fallback - return error so user knows AI failed
